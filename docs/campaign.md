...
Request request = new ListCampaigns(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .availableFrom("2024-04-19T01:56:08.000000Z") // 指定された日時以降に適用可能期間が含まれているか
        .availableTo("2020-07-17T12:10:08.000000Z") // 指定された日時以前に適用可能期間が含まれているか
        .page(1)                                  // ページ番号
...
Request request = new CreateCampaign(
    ...
    "topup"                                       // event: イベント種別
        ...
        .pointExpiresAt("2021-05-31T09:35:01.000000Z") // ポイント有効期限(絶対日時指定)
        .pointExpiresInDays(9782)                 // ポイント有効期限(相対日数指定)
        ...
        .applicableDaysOfWeek(new Integer[]{2,1}) // キャンペーンを適用する曜日 (複数指定)
        ...
        .minimumNumberOfProducts(6494)            // キャンペーンを適用する1会計内の商品個数の下限
        .minimumNumberOfAmount(3539)              // キャンペーンを適用する1会計内の商品総額の下限
        .minimumNumberForCombinationPurchase(7634) // 複数種類の商品を同時購入するときの商品種別数の下限
        .existInEachProductGroups(false)          // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        .maxPointAmount(4392)                     // キャンペーンによって付与されるポイントの上限
        .maxTotalPointAmount(9585)                // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        ...
        .budgetCapsAmount(1369986438);            // キャンペーン予算上限
...
Request request = new UpdateCampaign(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // campaignId: キャンペーンID
        ...
        .startsAt("2022-09-05T09:29:13.000000Z")  // キャンペーン開始日時
        .endsAt("2022-10-15T22:04:01.000000Z")    // キャンペーン終了日時
        ...
        .pointExpiresAt("2024-06-15T10:34:33.000000Z") // ポイント有効期限(絶対日時指定)
        .pointExpiresInDays(3096)                 // ポイント有効期限(相対日数指定)
        ...
        .applicableDaysOfWeek(new Integer[]{4})   // キャンペーンを適用する曜日 (複数指定)
        ...
        .blacklistedShopIds(new String[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"}) // キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
        .minimumNumberOfProducts(1212)            // キャンペーンを適用する1会計内の商品個数の下限
        .minimumNumberOfAmount(3117)              // キャンペーンを適用する1会計内の商品総額の下限
        .minimumNumberForCombinationPurchase(6096) // 複数種類の商品を同時購入するときの商品種別数の下限
        .existInEachProductGroups(true)           // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        .maxPointAmount(2013)                     // キャンペーンによって付与されるポイントの上限
        .maxTotalPointAmount(4132)                // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        .applicableAccountMetadata(applicable_account_metadata) // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        .applicableTransactionMetadata(applicable_transaction_metadata) // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        .budgetCapsAmount(1167364432);            // キャンペーン予算上限
