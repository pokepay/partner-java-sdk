...
Request request = new ListCampaigns(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .availableFrom("2022-01-21T09:38:31.000000+09:00") // 指定された日時以降に適用可能期間が含まれているか
        .availableTo("2020-11-17T08:18:30.000000+09:00") // 指定された日時以前に適用可能期間が含まれているか
        .page(1)                                  // ページ番号
...
Request request = new CreateCampaign(
    "s2d5vRgcGbKeZlZqB2psUdLmlh8aw5ZId4VPNDmzj290wJZkzSMfmtvFUfDQxuBXN034ZdoLd64i80zZ33xQziiqa3EuSkQAUcwvDgT4VE1LicnwFGMcMyXtfVnAGd5C02Rta2gHljaDtbS4yxYgEhmVh6zYo3fbXHgMfqJ0cHvGZQlz4M6rl1f7UuygJ41S0I6STgnvSdLhSR97TXnFdVK4sfcLhCaqXczcdStagg1vG4fRj4tj69kAkxclpPt", // name: キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    "2021-06-13T09:03:43.000000+09:00",           // startsAt: キャンペーン開始日時
    "2022-06-10T01:57:25.000000+09:00",           // endsAt: キャンペーン終了日時
    5664,                                         // priority: キャンペーンの適用優先度
    "topup"                                       // event: イベント種別
)
        .bearPointShopId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // ポイント負担先店舗ID
        .description("7n8GmpifcECcbW0mlgUX28FFq2j9lS8PpiuVFdUMowgIab78cSG99s9VpEbQ76OqR8Q7tVmvUSDE8mmrs55OMJeCHwasrMHKJT3uPhUS0cVlVaPnE3abZXqCvAKmg") // キャンペーンの説明文
        .status("disabled")                       // キャンペーン作成時の状態
        .pointExpiresAt("2020-12-30T07:34:32.000000+09:00") // ポイント有効期限(絶対日時指定)
        .pointExpiresInDays(9448)                 // ポイント有効期限(相対日数指定)
        ...
        .applicableDaysOfWeek(new Integer[]{5,4}) // キャンペーンを適用する曜日 (複数指定)
        ...
        .minimumNumberOfProducts(8536)            // キャンペーンを適用する1会計内の商品個数の下限
        .minimumNumberOfAmount(5348)              // キャンペーンを適用する1会計内の商品総額の下限
        .minimumNumberForCombinationPurchase(7282) // 複数種類の商品を同時購入するときの商品種別数の下限
        .existInEachProductGroups(true)           // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        .maxPointAmount(3623)                     // キャンペーンによって付与されるポイントの上限
        .maxTotalPointAmount(5043)                // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        ...
        .budgetCapsAmount(885643579);             // キャンペーン予算上限
...
Request request = new UpdateCampaign(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // campaignId: キャンペーンID
        ...
        .name("4vmDTPYLXv1frw5wyVC5qvr6o4FCT5j8rsjY9cpv34s2hTCU0SneIvVSZBVxygSowk3kXVOQTBeh2r4sBu7kVFQMdc4NDeFNhEdkkj41P3GGDEQwvqCnVv8CKTGrsu") // キャンペーン名
        .startsAt("2022-03-14T11:06:32.000000+09:00") // キャンペーン開始日時
        .endsAt("2021-03-12T07:00:54.000000+09:00") // キャンペーン終了日時
        .priority(3325)                           // キャンペーンの適用優先度
        .event("topup")                           // イベント種別
        .description("gxXwZPxWmz1tdEF59TL9pgEDK6ugnLWbg2Tb6fGTjM9R1OyQgYICs6ngcxphPe9ueURNiFDfV0lv3VIztjUGxzOanFtlgDVs2J9gua21r3lDUrK9nuMkug7Gp37bh8ChsBYqLBmgk8KD5RoSdq5") // キャンペーンの説明文
        .status("disabled")                       // キャンペーン作成時の状態
        .pointExpiresAt("2023-07-12T06:55:59.000000+09:00") // ポイント有効期限(絶対日時指定)
        .pointExpiresInDays(1517)                 // ポイント有効期限(相対日数指定)
        .setExclusive(false)                      // キャンペーンの重複設定
        .subject("money")                         // ポイント付与の対象金額の種別
        .amountBasedPointRules(new Object[]{items,items2,items3}) // 取引金額ベースのポイント付与ルール
        .productBasedPointRules(new Object[]{items4,items5}) // 商品情報ベースのポイント付与ルール
        .blacklistedProductRules(new Object[]{items6,items7,items8}) // 商品情報ベースのキャンペーンで除外対象にする商品リスト
        .applicableDaysOfWeek(new Integer[]{3,1,1}) // キャンペーンを適用する曜日 (複数指定)
        .applicableTimeRanges(new Object[]{items9}) // キャンペーンを適用する時間帯 (複数指定)
        .applicableShopIds(new String[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"}) // キャンペーン適用対象となる店舗IDのリスト
        .minimumNumberOfProducts(7531)            // キャンペーンを適用する1会計内の商品個数の下限
        .minimumNumberOfAmount(4654)              // キャンペーンを適用する1会計内の商品総額の下限
        .minimumNumberForCombinationPurchase(4748) // 複数種類の商品を同時購入するときの商品種別数の下限
        .existInEachProductGroups(true)           // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        .maxPointAmount(2469)                     // キャンペーンによって付与されるポイントの上限
        .maxTotalPointAmount(7845)                // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        .applicableAccountMetadata(applicable_account_metadata) // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        .applicableTransactionMetadata(applicable_transaction_metadata) // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        .budgetCapsAmount(1993237748);            // キャンペーン予算上限
