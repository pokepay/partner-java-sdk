...
Request request = new ListChecks()
        ...
        .organizationCode("DazAf")                // 組織コード
        .expiresFrom("2023-04-20T00:53:32.000000Z") // 有効期限の期間によるフィルター(開始時点)
        .expiresTo("2023-12-03T18:01:03.000000Z") // 有効期限の期間によるフィルター(終了時点)
        .createdFrom("2022-01-26T00:25:32.000000Z") // 作成日時の期間によるフィルター(開始時点)
        .createdTo("2020-09-07T00:32:24.000000Z") // 作成日時の期間によるフィルター(終了時点)
...
Request request = new CreateCheck(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // accountId: 送金元の店舗アカウントID
        ...
        .moneyAmount(301.0)                       // 付与マネー額
        .pointAmount(4188.0)                      // 付与ポイント額
        .description("test check")                // 説明文(アプリ上で取引の説明文として表示される)
        .setOnetime(false)                        // ワンタイムかどうかのフラグ
        .usageLimit(8236)                         // ワンタイムでない場合の最大読み取り回数
        .expiresAt("2021-01-31T22:29:59.000000Z") // チャージQRコード自体の失効日時
        .pointExpiresAt("2022-10-28T11:37:53.000000Z") // チャージQRコードによって付与されるポイント残高の有効期限
...
Request request = new UpdateCheck(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // checkId: チャージQRコードのID
        ...
        .moneyAmount(4136.0)                      // 付与マネー額
        .pointAmount(6857.0)                      // 付与ポイント額
        .description("test check")                // チャージQRコードの説明文
        .setOnetime(false)                        // ワンタイムかどうかのフラグ
        .usageLimit(3052)                         // ワンタイムでない場合の最大読み取り回数
        .expiresAt("2020-12-15T17:24:57.000000Z") // チャージQRコード自体の失効日時
        .pointExpiresAt("2021-11-28T19:59:09.000000Z") // チャージQRコードによって付与されるポイント残高の有効期限
        .pointExpiresInDays(60)                   // チャージQRコードによって付与されるポイント残高の有効期限(相対日数指定)
        .bearPointAccount("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // ポイント額を負担する店舗のウォレットID
