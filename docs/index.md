...
Request request = new GetCpmToken(
    "QWzLnud0F094pLPhlmaGDd"                      // cpmToken: CPMトークン
);
```
...
Request request = new ListTransactions()
        .from("2020-08-14T20:21:25.000000+09:00") // 開始日時
        .to("2023-03-13T11:58:46.000000+09:00")   // 終了日時
        .page(1)                                  // ページ番号
...
Request request = new CreateTransaction(
    ...
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
        ...
        .moneyAmount(9419)
        .pointAmount(4502)
        .pointExpiresAt("2020-06-30T18:19:05.000000+09:00") // ポイント有効期限
        .description("aA9xHz5tPjJOV34Fz2QibK5H4aBPmwwp7pktjDkoyHhUL4P4f2ZCnstSfcBhP6JVOPSeMSAtpl41t3NcQBqtaPcLOzmho148pdry8OUqAMsOKOEIEr0Q6jF7vzoIU");
```
...
Request request = new ListTransactionsV2()
        ...
        .customerName("太郎")                       // エンドユーザー名
        .description("店頭QRコードによる支払い")             // 取引説明文
        .transactionId("X4LA")                    // 取引ID
        .setModified(true)                        // キャンセルフラグ
        .types(new String[]{"topup","payment"})   // 取引種別 (複数指定可)、チャージ=topup、支払い=payment
        .from("2021-01-31T06:57:36.000000+09:00") // 開始日時
        .to("2021-03-28T08:48:41.000000+09:00")   // 終了日時
...
Request request = new CreateTopupTransaction(
    ...
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .moneyAmount(945)                         // マネー額
        .pointAmount(1873)                        // ポイント額
        .pointExpiresAt("2021-05-12T19:02:27.000000+09:00") // ポイント有効期限
...
Request request = new CreateCpmTransaction(
    "c5SNqrwOM77M6SD3csmTLG",                     // cpmToken: CPMトークン
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // shopId: 店舗ID
    3276.0                                        // amount: 取引金額
)
...
Request request = new CreateExchangeTransaction(
    ...
    1246
        ...
        .description("iB3ym7ZWnSq5SxsDWMKNINPZGST5TYjMjPCjwPYRBR5J6IP5dqnfV9JmSWaoMbWSoI5hTyahMC0XZeYP0SJK3OqYaIhT")
        .requestId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"); // リクエストID
```
...
Request request = new RefundTransaction(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // transactionId: 取引ID
        ...
        .returningPointExpiresAt("2020-11-25T23:49:12.000000+09:00"); // 返却ポイントの有効期限
```
...
Request request = new GetAccountTransferSummary(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // accountId: ウォレットID
        ...
        .from("2023-11-05T10:41:45.000000+09:00") // 集計期間の開始時刻
        .to("2022-12-30T18:46:52.000000+09:00")   // 集計期間の終了時刻
...
Request request = new ListTransfers()
        .from("2022-12-10T07:18:36.000000+09:00")
        .to("2021-03-25T21:11:50.000000+09:00")
        .page(321)
        .perPage(757)
        .shopId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
        .shopName("tWgDDhEAMq8lF0iP1UCgGkKsL0miwssuez5X2Gl5USMxqJahvEJW79um6lHbSoUzjsCHowOwlkONUSlqPGsqF6ho0b1IAaguJpvQ1kDeKVspGsOOLIiNRlXDskbFGiplSuCt")
        .customerId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
        .customerName("YI6M3VOykzD0cWnSq46xrQw7LYjwYJ0eYHThwNgDEDDjd9SnQPvNH2JbsvQVbRY3zjE7I")
        .transactionId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
        .privateMoneyId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx")
        .setModified(false)
        .transactionTypes(new String[]{"exchange","cashback","transfer","expire"})
        .transferTypes(new String[]{"expire","coupon","topup","campaign","transfer","cashback","exchange"}) // 取引明細の種類でフィルターします。
...
Request request = new ListTransfersV2()
        .shopId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // 店舗ID
        .shopName("dxkAyqf6tiGF89kmBdTLV6K8RtxGhMLqFnARRM") // 店舗名
        .customerId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // エンドユーザーID
        .customerName("2aBYWsLkQBzKnxaHAV5R1ev96Ijknk334hKLRdSZtwbSo06Scy5f0lePGmt13FUkhWclG8ut3TTSaqYpB3Ofune6ktEJIilX9oNhQIbnyzyqWKMjM09E10jleDJfhVrZxWhfmIJbFWZFlRdsgsUSUsb84o5DiPn0APOnG6b9JexBDhuRS0yxsWjnlMitKTbRbkny5Qq3lQFEYiIxatjwCgAgULuylq9VSZ7") // エンドユーザー名
        .transactionId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // 取引ID
        .privateMoneyId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // マネーID
        .setModified(false)                       // キャンセルフラグ
        .transactionTypes(new String[]{"topup","transfer","exchange"}) // 取引種別 (複数指定可)、チャージ=topup、支払い=payment
        ...
        .perPage(50)                              // 1ページ分の取引数
        .transferTypes(new String[]{"exchange","payment","transfer","campaign","coupon","cashback"}) // 取引明細種別 (複数指定可)
        .description("店頭QRコードによる支払い")             // 取引詳細説明文
        .from("2020-02-25T02:27:12.000000+09:00") // 開始日時
        .to("2022-05-14T17:57:59.000000+09:00");  // 終了日時
```
...
Request request = new CreateCheck(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // accountId: 送金元の店舗アカウントID
        ...
        .moneyAmount(6247.0)                      // 付与マネー額
        .pointAmount(5085.0)                      // 付与ポイント額
        .description("test check")                // 説明文(アプリ上で取引の説明文として表示される)
        .setOnetime(false)                        // ワンタイムかどうかのフラグ
        .usageLimit(7951)                         // ワンタイムでない場合の最大読み取り回数
        .expiresAt("2023-02-28T19:39:41.000000+09:00") // チャージQRコード自体の失効日時
        .pointExpiresAt("2020-11-06T20:25:48.000000+09:00") // チャージQRコードによって付与されるポイント残高の有効期限
...
Request request = new ListChecks()
        ...
        .perPage(50)                              // 1ページの表示数
        .privateMoneyId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // マネーID
        .organizationCode("8zLlVL5G80aDIp8oLFeJP") // 組織コード
        .expiresFrom("2024-01-12T00:32:10.000000+09:00") // 有効期限の期間によるフィルター(開始時点)
        .expiresTo("2022-10-09T02:17:57.000000+09:00") // 有効期限の期間によるフィルター(終了時点)
        .createdFrom("2023-05-19T00:53:42.000000+09:00") // 作成日時の期間によるフィルター(開始時点)
        .createdTo("2023-01-22T19:08:29.000000+09:00") // 作成日時の期間によるフィルター(終了時点)
        .issuerShopId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // 発行店舗ID
        .description("iRpzNWclL")                 // チャージQRコードの説明文
...
Request request = new UpdateCheck(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // checkId: チャージQRコードのID
        ...
        .moneyAmount(723.0)                       // 付与マネー額
        .pointAmount(5898.0)                      // 付与ポイント額
        .description("test check")                // チャージQRコードの説明文
        .setOnetime(true)                         // ワンタイムかどうかのフラグ
        .usageLimit(6392)                         // ワンタイムでない場合の最大読み取り回数
        .expiresAt("2021-04-04T06:02:04.000000+09:00") // チャージQRコード自体の失効日時
        .pointExpiresAt("2021-07-26T01:30:39.000000+09:00") // チャージQRコードによって付与されるポイント残高の有効期限
        .pointExpiresInDays(60)                   // チャージQRコードによって付与されるポイント残高の有効期限(相対日数指定)
        .bearPointAccount("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // ポイント額を負担する店舗のウォレットID
...
Request request = new ListBills()
        .page(924)                                // ページ番号
        .perPage(3088)                            // 1ページの表示数
        .billId("D68")                            // 支払いQRコードのID
        .privateMoneyId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // マネーID
        .organizationCode("-h-L476j-4-S2Pd--QaF--j") // 組織コード
        .description("test bill")                 // 取引説明文
        .createdFrom("2022-10-17T21:24:12.000000+09:00") // 作成日時(起点)
        .createdTo("2023-04-28T12:24:56.000000+09:00") // 作成日時(終点)
        .shopName("bill test shop1")              // 店舗名
        .shopId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // 店舗ID
        .lowerLimitAmount(4163)                   // 金額の範囲によるフィルタ(下限)
        .upperLimitAmount(94)                     // 金額の範囲によるフィルタ(上限)
...
Request request = new ListAccountBalances(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // accountId: ウォレットID
        ...
        .page(5077)                               // ページ番号
        .perPage(9572)                            // 1ページ分の取引数
        .expiresAtFrom("2021-06-07T11:38:15.000000+09:00") // 有効期限の期間によるフィルター(開始時点)
        .expiresAtTo("2023-11-28T16:06:54.000000+09:00") // 有効期限の期間によるフィルター(終了時点)
...
Request request = new ListAccountExpiredBalances(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // accountId: ウォレットID
        ...
        .page(5276)                               // ページ番号
        .perPage(5311)                            // 1ページ分の取引数
        .expiresAtFrom("2020-03-27T14:38:29.000000+09:00") // 有効期限の期間によるフィルター(開始時点)
        .expiresAtTo("2022-06-17T22:57:05.000000+09:00") // 有効期限の期間によるフィルター(終了時点)
...
Request request = new UpdateCustomerAccount(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // accountId: ウォレットID
        ...
        .status("pre-closed")                     // ウォレット状態
        .accountName("2V5wkKSc1L1GqfFYqIe2OhMAQ6sFQVTr8ip8e9p1klONHzGA9USEoct9dlVLFSWp50nekcdf5gPZuC7GZbar2UnGb0qxW45ye3z5hOCJJbAUIziJZCv5cGfnQSJTrZoL8FKX9W7TxA9Np2l7OMwDzHzH57q8OlMJ4D1KWVGNx") // アカウント名
        .externalId("UCOIohOet0NiTIlmdJmes8Kn6SChds0Fpos") // 外部ID
...
Request request = new GetCustomerAccounts(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .page(3116)                               // ページ番号
        .perPage(6711)                            // 1ページ分のウォレット数
        .createdAtFrom("2022-10-16T16:11:56.000000+09:00") // ウォレット作成日によるフィルター(開始時点)
        .createdAtTo("2021-11-28T04:37:01.000000+09:00") // ウォレット作成日によるフィルター(終了時点)
        .setSuspended(true)                       // ウォレットが凍結状態かどうかでフィルターする
        .status("suspended")                      // ウォレット状態
        .externalId("xg4puxtcTae5R2QQFKsqDPTrcsCnWtnG0xAIKd") // 外部ID
        .tel("058991-3214")                       // エンドユーザーの電話番号
        .email("H6C7imrgIM@MeAL.com");            // エンドユーザーのメールアドレス
```
...
Request request = new CreateCustomerAccount(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .externalId("flPMozSXmXtY1WT0lYDvSne6wpoxhBtfPrSz"); // 外部ID
```
...
Request request = new GetShopAccounts(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .page(5816)                               // ページ番号
        .perPage(752)                             // 1ページ分のウォレット数
        .createdAtFrom("2022-03-28T03:36:55.000000+09:00") // ウォレット作成日によるフィルター(開始時点)
        .createdAtTo("2020-05-24T05:59:03.000000+09:00") // ウォレット作成日によるフィルター(終了時点)
...
Request request = new ListCustomerTransactions(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .from("2023-05-07T20:47:58.000000+09:00") // 開始日時
        .to("2021-02-08T02:37:02.000000+09:00")   // 終了日時
        .page(1)                                  // ページ番号
        .perPage(50);                             // 1ページ分の取引数
```
...
Request request = new ListShops()
        ...
        .privateMoneyId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // マネーID
        .name("oxスーパー三田店")                        // 店舗名
        .postalCode("789-4704")                   // 店舗の郵便番号
        .address("東京都港区芝...")                     // 店舗の住所
        .tel("07-7741-8327")                      // 店舗の電話番号
        .email("obYNq21I8j@YVu2.com")             // 店舗のメールアドレス
        .externalId("iJF0gwsC6sbIjU9zvDXlKXA1Lu2kz6FsJH") // 店舗の外部ID
...
Request request = new CreateShop(
    "oxスーパー三田店"                                   // shopName: 店舗名
        ...
        .shopPostalCode("425-1184")               // 店舗の郵便番号
        .shopAddress("東京都港区芝...")                 // 店舗の住所
        .shopTel("0387-474-624")                  // 店舗の電話番号
        .shopEmail("uhutsbyZHJ@35KE.com")         // 店舗のメールアドレス
        .shopExternalId("GwdDNBUfw3VUgPWWf2t14rFQyffyJ") // 店舗の外部ID
...
Request request = new CreateShopV2(
    "oxスーパー三田店"                                   // name: 店舗名
        ...
        .postalCode("756-5909")                   // 店舗の郵便番号
        .address("東京都港区芝...")                     // 店舗の住所
        .tel("0125483")                           // 店舗の電話番号
        .email("8PG8RdfCeO@IJ2b.com")             // 店舗のメールアドレス
        .externalId("mOTPYK")                     // 店舗の外部ID
        .organizationCode("ox-supermarket")       // 組織コード
...
Request request = new UpdateShop(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // shopId: 店舗ユーザーID
        ...
        .name("oxスーパー三田店")                        // 店舗名
        .postalCode("191-8435")                   // 店舗の郵便番号
        .address("東京都港区芝...")                     // 店舗の住所
        .tel("061-713665")                        // 店舗の電話番号
        .email("oF6RuFAt5L@LOqo.com")             // 店舗のメールアドレス
        .externalId("R1k5RAvXoZRJ31nSWlDLkLZuSaOXNMg") // 店舗の外部ID
...
Request request = new CreateUserAccount(
    ...
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .name("RA74RO5cr9lKAB2z3dMw9WDBwP1P1BIbKDYgGdjSBUbPRPWILYo0lgitZclc1PeoaT4Lvh9IPkUTGdPj8Jst8yjhnFBNqCeH2ED7OKt1zmSosDoYS49AxlJFyWfCmoHo1C4146FLDkL") // ウォレット名
        .externalId("UOUtDYp3B3Ke6X7kL4Y9cbuqvytkwxdTMIFVhqGnbCdTqw") // 外部ID
        .metadata("{\"key1\":\"foo\",\"key2\":\"bar\"}"); // ウォレットに付加するメタデータ
```
...
Request request = new GetPrivateMoneyOrganizationSummaries(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .from("2023-10-09T09:45:51.000000+09:00") // 開始日時(toと同時に指定する必要有)
        .to("2023-03-12T22:32:32.000000+09:00")   // 終了日時(fromと同時に指定する必要有)
        .page(1)                                  // ページ番号
        .perPage(50);                             // 1ページ分の取引数
```
...
Request request = new GetPrivateMoneySummary(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .from("2023-05-01T00:01:51.000000+09:00") // 開始日時
        .to("2020-12-07T17:05:06.000000+09:00");  // 終了日時
```
...
Request request = new BulkCreateTransaction(
    "6NasLXxxn",                                  // name: 一括取引タスク名
    "aH",                                         // content: 取引する情報のCSV
    "52MuuS2sJCX8vDPAdESSk5O9Ezw2vbH83t0b"        // requestId: リクエストID
)
        .description("EfLx6NJuPsaQhbtg1599ierHzZulTLsjTImKexd9T2Oij08K3T1ADnoq967eh8nvRpdzU2ZLNvaStEwjxw1o1xrVkOhrRb") // 一括取引の説明
        .privateMoneyId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"); // マネーID
```
...
Request request = new CreateCampaign(
    "oisc0nnotxhfoy4w5qpUxZBhGbiNRWC1XsupkxRCZoDleGG3mqkDsU5YSRh78gWP1D6ru3jmgcpfIl4b1Oc2L1UMALFhfvwGVrrG98S3v8Zlwk7JFO9kzb3UgfxIOhhVuanUJeIr55zGYUJZVlcQJ7r2G1xMJt1y3wZbD4zU8xolWUNLMH8oFcqLfdAODTeZXVHxLGFtjOVsr4mLhg6CB6WiWYfX9", // name: キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",       // privateMoneyId: マネーID
    "2022-08-02T07:45:29.000000+09:00",           // startsAt: キャンペーン開始日時
    "2023-04-24T11:09:02.000000+09:00",           // endsAt: キャンペーン終了日時
    7336,                                         // priority: キャンペーンの適用優先度
    "topup"                                       // event: イベント種別
)
        .bearPointShopId("xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx") // ポイント負担先店舗ID
        .description("5UDvJ6lNh1jcSKaIJ2GiDncmX4ewoNHiJdGKwFArvdlQQzlnz2Mg2aU5RQSeWwBbYqJ1U9OKWCs6Pe2c3fJHuk3xIUllzZVL2tRovxBVaqbBFg67yzK0mXHb36vxdl") // キャンペーンの説明文
        .status("disabled")                       // キャンペーン作成時の状態
        .pointExpiresAt("2023-09-04T06:07:51.000000+09:00") // ポイント有効期限(絶対日時指定)
        .pointExpiresInDays(5911)                 // ポイント有効期限(相対日数指定)
        ...
        .applicableDaysOfWeek(new Integer[]{0,5,4}) // キャンペーンを適用する曜日 (複数指定)
        ...
        .minimumNumberForCombinationPurchase(185) // 複数種類の商品を同時購入するときの商品種別数の下限
        .existInEachProductGroups(false)          // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        .maxPointAmount(7500)                     // キャンペーンによって付与されるポイントの上限
        .maxTotalPointAmount(6709)                // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        ...
        .budgetCapsAmount(1703913702);            // キャンペーン予算上限
```
...
Request request = new ListCampaigns(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: マネーID
        ...
        .availableFrom("2023-10-26T21:05:05.000000+09:00") // 指定された日時以降に適用可能期間が含まれているか
        .availableTo("2022-06-08T15:43:34.000000+09:00") // 指定された日時以前に適用可能期間が含まれているか
        .page(1)                                  // ページ番号
...
Request request = new UpdateCampaign(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // campaignId: キャンペーンID
        ...
        .name("iAVcFMxyZNA")                      // キャンペーン名
        .startsAt("2023-07-28T17:14:03.000000+09:00") // キャンペーン開始日時
        .endsAt("2020-10-14T02:13:20.000000+09:00") // キャンペーン終了日時
        .priority(1709)                           // キャンペーンの適用優先度
        .event("topup")                           // イベント種別
        .description("iwHaRbzMKfbFLvYsovByQZJnPKA4jT5Lp1NTcEcSfS65fFsJfcgg8upyMayYwwYY9SWXNyeDzZrwVQkajKaWM0a0ZTmZ6LmdPmsTjPAnK45CO0foCzjjSePPfH0k4IafcE") // キャンペーンの説明文
        .status("disabled")                       // キャンペーン作成時の状態
        .pointExpiresAt("2022-03-25T10:28:20.000000+09:00") // ポイント有効期限(絶対日時指定)
        .pointExpiresInDays(8807)                 // ポイント有効期限(相対日数指定)
        .setExclusive(false)                      // キャンペーンの重複設定
        .subject("all")                           // ポイント付与の対象金額の種別
        .amountBasedPointRules(new Object[]{items,items2,items3}) // 取引金額ベースのポイント付与ルール
        .productBasedPointRules(new Object[]{items4}) // 商品情報ベースのポイント付与ルール
        .applicableDaysOfWeek(new Integer[]{4,4,5}) // キャンペーンを適用する曜日 (複数指定)
        .applicableTimeRanges(new Object[]{items5}) // キャンペーンを適用する時間帯 (複数指定)
        .applicableShopIds(new String[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx","xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"}) // キャンペーン適用対象となる店舗IDのリスト
        .minimumNumberForCombinationPurchase(81)  // 複数種類の商品を同時購入するときの商品種別数の下限
        .existInEachProductGroups(false)          // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
        .maxPointAmount(7612)                     // キャンペーンによって付与されるポイントの上限
        .maxTotalPointAmount(5380)                // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
        .applicableAccountMetadata(applicable_account_metadata) // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
        .budgetCapsAmount(2081036770);            // キャンペーン予算上限
```
...
Request request = new ListCoupons(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // privateMoneyId: 対象クーポンのマネーID
        ...
        .couponId("MQmCKi")                       // クーポンID
        .couponName("4v")                         // クーポン名
        .issuedShopName("2774D06hL")              // 発行店舗名
        .availableShopName("Gya2WTe")             // 利用可能店舗名
        .availableFrom("2021-12-21T17:12:41.000000+09:00") // 利用可能期間 (開始日時)
        .availableTo("2020-07-25T00:55:09.000000+09:00") // 利用可能期間 (終了日時)
        .page(1)                                  // ページ番号
        .perPage(50);                             // 1ページ分の取得数
```
...
Request request = new CreateCoupon(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",
    "ybLrTaKvh5IhSI8t3rhh1PZANlvnM5EvQDA7r3bz1sGGi",
    "2021-12-09T20:49:26.000000+09:00",
    "2023-01-08T22:45:41.000000+09:00",
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // issuedShopId: 発行元の店舗ID
)
        .description("p9UkDfKRS69p8JnWwNEYpEOWpHwo738IMvCIrgAPJGtE8jwVatddHcvyLiqcND07DixC8wQMAV1plJB2R3QqH1c7IMpTWK6pFGWk5QpuPHuB9gx7sSxa5dGZ1GjZUXmWSFAujwt9JbZoPpGz04E0w7Q8ysa6z")
        .discountAmount(4361)
        .discountPercentage(4319.0)
        .discountUpperLimit(5850)
        .displayStartsAt("2021-10-23T20:36:06.000000+09:00") // クーポンの掲載期間(開始日時)
        .displayEndsAt("2020-01-07T07:43:55.000000+09:00") // クーポンの掲載期間(終了日時)
        .setDisabled(false)                       // 無効化フラグ
        .setHidden(false)                         // クーポン一覧に掲載されるかどうか
        .setPublic(false)                         // アプリ配信なしで受け取れるかどうか
        .code("znqGSZWsc")                        // クーポン受け取りコード
        .usageLimit(5226)                         // ユーザごとの利用可能回数(NULLの場合は無制限)
        .minAmount(8061)                          // クーポン適用可能な最小取引額
...
Request request = new UpdateCoupon(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"        // couponId: クーポンID
        ...
        .name("Wn5y0lwe33p3vr2R9fJn74DjUu0kmjp1f0E1aHN8yP9bOkTBp9A8qxTIm6bkP5GMM3j40")
        .description("93k7V59gCxmsW7ZEVznXd8aUj22ucbNTJcjasJBsV9ivXRx8bVoz7FoF9QFgPQteRl5G3YxPGeztidl1i4fYtK8sQLrfeaqlyzEgJa6wtgSthfsgs8vLX4pXhuBugSGFnTDHNve2RcbFbxSMY9JpHzggdPlt63GQqtRicRo3o6YWbMY6V1Pk3ddzvhSi58Q4goFPJdKKyvokzXAdxAKCSRYv54ENlvuY7J1qLLzXTM4iEg")
        .discountAmount(9309)
        .discountPercentage(5726.0)
        .discountUpperLimit(6950)
        .startsAt("2022-11-10T11:23:54.000000+09:00")
        .endsAt("2022-11-04T21:17:28.000000+09:00")
        .displayStartsAt("2021-09-30T00:09:55.000000+09:00") // クーポンの掲載期間(開始日時)
        .displayEndsAt("2020-10-10T10:05:44.000000+09:00") // クーポンの掲載期間(終了日時)
        .setDisabled(false)                       // 無効化フラグ
        .setHidden(true)                          // クーポン一覧に掲載されるかどうか
        .setPublic(true)                          // アプリ配信なしで受け取れるかどうか
        .code("gYCcKtu")                          // クーポン受け取りコード
        .usageLimit(8922)                         // ユーザごとの利用可能回数(NULLの場合は無制限)
        .minAmount(2942)                          // クーポン適用可能な最小取引額
...
Request request = new CreateBank(
    ...
    "ポケペイタロウ"                                     // kana: ユーザーの氏名 (片仮名で指定)
        ...
        .email("djuNvigRdi@1GQ1.com")             // ユーザーのメールアドレス
